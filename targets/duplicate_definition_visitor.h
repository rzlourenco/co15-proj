...
namespace pwn {
  class duplicate_definition_visitor : public basic_ast_visitor {
    std::vector<std::map<std::string, bool>> context;

    void push() {
      // Places a default-constructed std::set<std::string> at the end of context
      context.emplace_back();
    }

    void declare(const std::string &id) {
      auto it = context.back().find(id);
      if (it != context.back().end() && it->second)
        throw id + " was already defined in this scope";

      context.back()[id] = false;
    }

    void define(const std::string &id) {
      auto it = context.back().find(id);
      if (it != context.back().end() && it->second)
        throw id + " was already defined in this scope";

      context.back()[id] = true;
    }
    ...
    public:
    duplicate_definition_visitor(std::shared_ptr<cdk::compiler> compiler)
      : basic_ast_visitor(compiler) {
        // There must be at least one set (globals)
        context.emplace_back();
      }
